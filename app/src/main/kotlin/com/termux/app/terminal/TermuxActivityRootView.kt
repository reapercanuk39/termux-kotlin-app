...
/**
 * The [TermuxActivity] relies on [android.view.WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE]
 * set by [TermuxTerminalViewClient.setSoftKeyboardState] to automatically
 ...
 * and [com.termux.view.TerminalView.onCreateInputConnection] sets the inputType
 ...
 * part of the candidates view returned by the keyboard app in [android.inputmethodservice.InputMethodService.onCreateCandidatesView].
 *
 ...
 * activity theme. When [TermuxActivity] [ViewTreeObserver.OnGlobalLayoutListener] is
 ...
 */
open class TermuxActivityRootView @JvmOverloads constructor(
    context: Context,
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        marginBottom?.let { margin ->
            if (ROOT_VIEW_LOGGING_ENABLED)
                Logger.logVerbose(LOG_TAG, "onMeasure: Setting bottom margin to $margin")
    ...
    override fun onGlobalLayout() {
        ...
        val bottomSpaceView = activity.termuxActivityBottomSpaceView ?: return

        val rootViewLoggingEnabled = ROOT_VIEW_LOGGING_ENABLED

        if (rootViewLoggingEnabled)
        ...
        //val isVisible = windowAvailableRect.contains(bottomSpaceViewRect) // rect.right comparison often fails in landscape
        ...
        if (isVisible) {
            ...
            if (setMargin) {
                ...
                // This is done since we **expect** the keyboard to have same dimensions next time
                // layout changes, so best set margin while view is drawn the first time, otherwise it will
